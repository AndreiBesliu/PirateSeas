...
- **Porţile:**
  ...
  - `python tools/ci_measure.py` — 76 scenarii headless vs `tools/measurement_baseline.json`
...
- ~~**Fugara blocheaza victoria.**~~ **INCHIS 26.09:** fugara (6,42 m/s drept in
  pupa) merge exact cat urmaritorul, deci nu poate fi prinsa, doar tinuta la
  distanta. Scapa peste `EscapeRangeM` (2000 m) de coca vie a jucatorului, sau la
  `EscapeEdgeMarginM` (300 m) de marginea apei; iese de pe apa, iar
  `TallySquadron` (o singura socoteala, si pentru scufundare) da victoria.
  Perechea `escape_on`/`escape_off` plus garzile `escape_fighting`,
  `escape_player_down`, `escape_wreck`, `escape_edge`.
